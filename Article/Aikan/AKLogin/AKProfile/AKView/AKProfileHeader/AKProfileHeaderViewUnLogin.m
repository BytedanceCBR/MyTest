...
- (void)layoutSubviews
{
    ...
    self.topImageView.centerX = self.topRegion.width / 2;
    self.topImageView.top = topInset;
    self.topTitleLabel.centerX = self.topImageView.centerX;
    self.topTitleLabel.top = self.topImageView.bottom + 10;
...
- (void)createTopRegion
{
    ...
    imageView.image = [UIImage imageNamed:@"profile_unlogin_header_image"];
    imageView.frame = CGRectMake(0, 0, [TTDeviceUIUtils tt_newPadding:286], [TTDeviceUIUtils tt_newPadding:105]);
    ...
    topTitleLabel.text = @"“登录后现金红包天天领”";
    [topTitleLabel sizeToFit];
    [containerView addSubview:topTitleLabel];
    self.topTitleLabel = topTitleLabel;
    CGFloat containerViewHeight = 20.f + imageView.height + topTitleLabel.height + 10;
    if ([TTDeviceHelper isIPhoneXDevice]) {
