    ...
    func refresh() {
        ...
        itemViews = items.map { item -> SearchConditionItemView in
            ...
            if item.isHighlighted {
                ...
                result.setConditionLabelText(
                        label: item.label,
                        color: hexStringToUIColor(hex: "#081f33"),
                        icon: #imageLiteral(resourceName:"icon-triangle-open-gray"))
                result.triangle.image = UIImage(named: "icon-triangle-open-gray")
            }
            return result
        }
...
class SearchConditionItemView: UIView {
    ...
    lazy var triangle: UIImageView = {
        ...
        result.image = UIImage(named: "icon-triangle-open-gray")
        return result
    }()
    ...
    func setConditionLabelText(label: String, color: UIColor, icon: UIImage) {
        ...
        let attributes: [NSAttributedStringKey: Any] = [.font: CommonUIStyle.Font.pingFangRegular(14)]
...
//        if let data = try? JSONSerialization.data(withJSONObject: self.value, options: []),
//            let value = String(data: data, encoding: .utf8) {
            result = "\(type)[]=\(stringValueOfAny(self.value))"
//        }
