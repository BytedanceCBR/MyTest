...
open class PageableViewModel: NSObject {
    ...
    public init(
        ...
        let pageableScrollVIew = CyclePageScrollView()
        pageableScrollVIew.alwaysBounceHorizontal = false
        pageableScrollVIew.showsHorizontalScrollIndicator = false
        self.pageView = pageableScrollVIew
        self.cacheViewCount = cacheViewCount
        self.currentPage.accept(initPageIndex)
        ...
        DispatchQueue.main.async {
            ...
            self.activateCurrentPage(currentPage: self.currentPage.value)
            self.reloadPageView(
                currentPage: self.currentPage.value,
                cacheViewCount: self.cacheViewCount)
    ...
    func adjectPageLayoutWhenScrollEnded(pageView: UIScrollView) {
        pageView.rx.didScroll
            ...
            .subscribe(onNext: { [unowned self] () in
                self.currentPage.accept(self.currentPageByOffsetSize(contentOffset: pageView.contentOffset))
                self.setPageViewFrame(
                    currentPage: self.currentPage.value,
                    pageViewCache: self.pageViews,
                    cacheViewCount: self.cacheViewCount)
    ...
    func onPageLayout() {
        ...
        let offsetPosition = CGFloat(currentPage.value) * pageWidth + (CGFloat(bufferSize) * pageWidth)
    ...
    func changeToPerviousPage() {
        let frame = pageFrameOfIndex(pageIndex: self.currentPage.value - 1, pageWidth: pageView.frame.size.width, pageHeight: pageView.frame.height)
    ...
    func changeToPageAtIndex(index: Int, animated: Bool = false, onComplete: (() -> Void)? = nil) {
        ...
        UIView.animate(withDuration: 0.1, animations: {
            ...
            self.currentPage.accept(self.currentPageByOffsetSize(contentOffset: self.pageView.contentOffset))
            self.setPageViewFrame(
                currentPage: self.currentPage.value,
                pageViewCache: self.pageViews,
                cacheViewCount: self.cacheViewCount)
...
@objc
class CyclePageScrollView: UIScrollView {
