...
extension TracerParams {
    ...
    func exclude(_ key: String) -> TracerParams {
        return TracerParams { input in
            let datas = self.paramsGetter(input)
            let result = datas.filter { (e) in
                let (k, _) = e
                return k != key
            }
            return result
...
func selectTraceParam(_ params: TracerParams, key: String) -> Any? {
    return params.paramsGetter([:])[key]
}
