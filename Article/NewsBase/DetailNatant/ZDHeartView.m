...
#import "ZDHeartView.h"
#import "SSThemed.h"

@interface ZDHeartView ()

@property (nonatomic) UIView *waveView;
@end

@implementation ZDHeartView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.shapelayer.fillColor = SSGetThemedColorWithKey(kColorText4).CGColor;
        [self.waveView.layer addSublayer:self.shapelayer];

        // mask
        UIImage *image = [UIImage resourceImageNamed:@"likeicon_actionbar_details_press.png"];
        ...
        //顶上的图 灰色心型框
        UIImage *imageTop = [UIImage resourceImageNamed:@"likeicon_actionbar_details.png"];
...
-(void)doAnimationWithAppendAnimation:(ZDHeartAnimationBlock)animation
                           completion:(ZDHeartAnimationBlock)completion
{
    if (!_hasLiked) {
...
{
    self.imageViewBase.image = [UIImage resourceImageNamed:@"likeicon_actionbar_details_press.png"];
    self.imageViewTop.image = [UIImage resourceImageNamed:@"likeicon_actionbar_details.png"];
    self.shapelayer.fillColor = SSGetThemedColorWithKey(kColorText4).CGColor;
}
