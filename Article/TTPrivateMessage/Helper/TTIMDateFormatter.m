...
{
    if (!message.sendDate) { return nil; }
    
    // 为空串@""或有值，说明计算过，返回缓存；只有为nil时做计算
    if (message.formattedSendDate) {
        return message.formattedSendDate;
    }
    if (!lastMsg || (lastMsg.sendDate && (message.sendDate.timeIntervalSince1970 - lastMsg.sendDate.timeIntervalSince1970) >= kThresholdTimeInterval)) {
        return [[[self class] sharedInstance] formattedDateWithSourceDate:message.sendDate showTime:YES];
    }
    return @"";
}
