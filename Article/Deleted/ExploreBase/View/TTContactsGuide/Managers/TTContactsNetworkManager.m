...
const TTContactProperty kTTContactsOfPropertiesV2 = kTTContactPropertyFullName | kTTContactPropertyNickname | kTTContactPropertyOrganizationName | kTTContactPropertyNote | kTTContactPropertyPhoneNumbers | kTTContactPropertyEmailAddresses | kTTContactPropertyUrlAddresses | kTTContactPropertyInstantMessageAddresses | kTTContactPropertySocialProfiles;

@implementation TTContactsNetworkManager

+ (void)requestHasUploadedContactsWithCompletion:(void (^)(NSError *error, BOOL hasUploadedContacts))completion {
    FRUserRelationContactinfoRequestModel *requestModel =  [[FRUserRelationContactinfoRequestModel alloc] init];
    ...
    [[TTNetworkManager shareInstance] requestModel:requestModel callback:^(NSError *error, NSObject<TTResponseModelProtocol> *responseModel) {
        ...
        FRUserRelationContactinfoResponseModel *model = (FRUserRelationContactinfoResponseModel *)responseModel;

        // message 只有等于 success 时，其他值才有意义
        if (![model.message isEqualToString:@"success"]) {
            if (completion) {
                NSError *serverError = [NSError errorWithDomain:kTTNetworkErrorDomain code:TTNetworkErrorCodeServerError userInfo:nil];
                completion(serverError, NO);
            }
            return;
        }

        if (completion) {
            completion(nil, model.is_collected.boolValue);
        }
    }];
}
...
+ (void)postContacts:(NSArray<TTABContact *> *)contacts userActive:(BOOL)userActive completion:(void (^)(NSError *error, id jsonObj))completion {
    ...
    [postParameters setValue:[NSNumber numberWithInteger:1] forKey:@"watermark"];
    ...
    [[TTNetworkManager shareInstance] uploadWithURL:[ArticleURLSetting uploadAddressBookV2URLString] parameters:postParameters constructingBodyWithBlock:^(id<TTMultipartFormData> formData) {
        /**
         *  key Generator
         */
