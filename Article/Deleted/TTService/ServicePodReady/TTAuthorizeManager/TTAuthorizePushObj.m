 ...
 **iOS，如之前已关闭权限，点击开启后弹窗提示：
 iOS8以上版本：「开启推送失败，请去设置项允许头条推送通知」，展示「取消/去设置」选项
 iOS8以下版本：「开启推送失败，请去 设置-通知 内开启头条访问权限」
 */
...
- (void)showAlertAtActionFeedRefreshWithCompletion:(dispatch_block_t)completionHandler sysAuthFlag:(NSInteger)flag {
    void (^pushAuthorizeSettingBlock)() = ^{
        ...
        NSString *defaultMessage = canOpenSettings ? @"第一时间获取重大新闻，去设置中允许头条通知" : @"第一时间获取重大新闻，请去 设置-通知 内开启头条访问权限";
        ...
        _hintView = [self authorizeHintViewWithTitle:titleString message:message image:imageObject okButtonTitle:okBtnTitle okBlock:^{
            if (canOpenSettings) {
                ...
                if ([TTDeviceHelper OSVersionNumber] >= 10.0 && [[UIApplication sharedApplication] respondsToSelector:@selector(openURL:options:completionHandler:)]) {
