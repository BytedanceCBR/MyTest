...
- (instancetype)initWithFrame:(CGRect)frame style:(TTCategorySelectorViewStyle)aStyle tabType:(TTCategorySelectorViewTabType)tabType textColors:(NSArray<NSString *> *)textColors textGlowColors:(NSArray<NSString *> *)textGlowColors textGlowSize:(CGFloat)glowSize
{
    ...
    if(self)
    {
        ...
        if ([TTThemeManager sharedInstance_tt].currentThemeMode == TTThemeModeDay && [TTSurfaceManager resurfaceEnable]){
            _maskTitleLabel.textColor = [TTSurfaceManager currentSurfaceColor];
            _maskTitleLabel.glowColor = [TTSurfaceManager currentSurfaceColor];
        }
        
        _titleLabel.font = [UIFont systemFontOfSize:[TTCategorySelectorView channelFontSizeWithStyle:aStyle tabType:tabType]];
        
        _maskTitleLabel.font = _titleLabel.font;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(themeChanged:)
                                                   ...
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(surfaceChangeNotification:)
                                                     name:kTTSurfaceChangeNotification
                                                   object:nil];
    }
...
- (void)surfaceChangeNotification:(NSNotification *)notification
{
    if ([TTThemeManager sharedInstance_tt].currentThemeMode == TTThemeModeDay && [TTSurfaceManager resurfaceEnable]){
        _maskTitleLabel.textColor = [TTSurfaceManager currentSurfaceColor];
        _maskTitleLabel.glowColor = [TTSurfaceManager currentSurfaceColor];
                      ...
                      tabType:(TTCategorySelectorViewTabType)tabType
{
    ...
    if(self)
    {
        ...
        } else if (style == TTCategorySelectorViewLightStyle) {
...
//            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveBadgeRefreshedNotification:) name:kArticleBadgeManagerRefreshedNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showCategoryGuide:) name:@"kAppShowCategoryGuide" object:nil];
...
- (void)refreshExpandButton:(NSNotification *)notification
{
    ...
    if (self.style == TTCategorySelectorViewVideoStyle || [TTDeviceHelper isPadDevice]) {
        ...
        self.scrollView.contentInset = UIEdgeInsetsMake(0, 0, 0, self.expandButton.hidden ? 0 : self.rightBorderIndicatorView.width);
    }
}
...
//    }
    ...
    if (self.tabType == TTCategorySelectorViewNewsTab && self.style == TTCategorySelectorViewLightStyle && ![TTDeviceHelper isPadDevice])
    {
        margin = 16;
...
- (void)categoryClicked:(CategorySelectorButton*)sender
{
    ...
    if(idx != NSNotFound && idx < _categories.count)
    {
        ...
        if (category.categoryID) {
            CLS_LOG(@"categoryClicked=%@",category.categoryID);
        }
...
- (void)refreshSelectorView
{
    ...
    for(; idx < _categories.count; idx ++)
    {
        ...
        if (self.style == TTCategorySelectorViewVideoStyle && [category.displayName length] > 0) {
            [categoryButton setText:category.displayName];
        } else {
        ...
        if(!_lastSelectedButton && [category.categoryID isEqualToString:kTTMainCategoryID])
        {
...
{
    if(fromIndex < _categories.count && toIndex < _categories.count && fromIndex >= 0 && toIndex >= 0 && fromIndex != toIndex)
    {
...
- (void)showTipNewBadge:(BOOL)showBadge categoryID:(NSString *)categoryID
{
    if(showBadge)
    {
        ...
        if (self.style != TTCategorySelectorViewVideoStyle) {
...
- (void)handleInsertCategoryIntoLastVisiblePosition:(NSNotification*)notification
{
    ...
    NSMutableArray *categories =  [NSMutableArray arrayWithArray:[[TTArticleCategoryManager sharedManager] subScribedCategories]];
...
- (void)showCategoryGuide:(NSNotification *)notification
{
    //频道引导动画
    CGFloat preLeft = self.scrollView.left;
    self.scrollView.left = [TTUIResponderHelper mainWindow].width;
    [UIView animateWithDuration:1.4 delay:0.5 usingSpringWithDamping:0.6 initialSpringVelocity:0.2 options:0 animations:^{
        self.scrollView.left = preLeft;
    } completion:nil];
}
...
- (NSInteger)lastVisibleCategoryIndex
{
    ...
    [_scrollView.subviews enumerateObjectsUsingBlock:^(id subview, NSUInteger idx, BOOL *stop) {
        if([subview isKindOfClass:[CategorySelectorButton class]])
        {
            ...
            if(button.right < _scrollView.width)
            {
                result = MAX(button.categoryModel.orderIndex, result);
            }
        }
    }];
