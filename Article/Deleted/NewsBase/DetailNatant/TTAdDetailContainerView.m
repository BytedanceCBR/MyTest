...
- (void)reloadSubViews {
    ...
    __block BOOL isMovieViewOnADView = NO;
    __block ExploreMovieView *exploreMovieView = nil;
    ...
    [self.adViews enumerateObjectsUsingBlock:^(ExploreDetailBaseADView  *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[ExploreDetailMixedVideoADView class]]) {
            ...
            if (videoADView.movieView) {
                exploreMovieView = videoADView.movieView;
                if ([exploreMovieView superview] == videoADView) {
                    isMovieViewOnADView = YES;
                }
                videoADView.movieView = nil;
            }
        }
    }];
    ...
    [self.adModels enumerateObjectsUsingBlock:^(ArticleDetailADModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (TTClassIsSubClassOfClass(cls, ExploreDetailBaseADView.class)) {
            ExploreDetailBaseADView *adView = [[cls alloc] initWithWidth:self.width];
            ...
            if (!isEmptyString(exploreMovieView.videoID) && !isEmptyString(adView.adModel.videoInfo.videoID) && [adView.adModel.videoInfo.videoID isEqualToString:exploreMovieView.videoID]) {
                if ([adView isKindOfClass:[ExploreDetailMixedVideoADView class]]) {
                    ExploreDetailMixedVideoADView *videoADView = (ExploreDetailMixedVideoADView *)adView;
                    if (isMovieViewOnADView) {
                        exploreMovieView.frame = videoADView.logo.bounds;
                        [videoADView addSubview:exploreMovieView];
                    }
                    exploreMovieView.movieFatherView = videoADView;
                    exploreMovieView.movieInFatherViewFrame = videoADView.logo.bounds;
                    videoADView.movieView = exploreMovieView;
                    videoADView.movieViewDelegate = [[ExploreArticleMovieViewDelegate alloc] init];
                    videoADView.movieViewDelegate.logo = videoADView.logo;
                    exploreMovieView.movieViewDelegate = videoADView.movieViewDelegate;
                }
            }
        } else {
    ...
    [self.adModels enumerateObjectsUsingBlock:^(ArticleDetailADModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (![self.showEvents containsObject:ad_id]) {
            if (obj.detailADType == ArticleDetailADModelTypePhone) {
                ...
                NSString *mediaID = [self.viewModel.article.mediaInfo[@"media_id"] stringValue];
