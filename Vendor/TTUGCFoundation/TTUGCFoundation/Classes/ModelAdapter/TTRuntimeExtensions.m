...
tt_propertyAttributes *tt_copyPropertyAttributes (objc_property_t property) {
    ...
    tt_propertyAttributes *attributes = calloc(1, sizeof(tt_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate tt_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = tt_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = tt_propertyMemoryManagementPolicyRetain;
            break;
