...
- (void)addGesture {
    if ([TTKitchen getBOOL:kTTKUGCBrowserQRCode]) {
...
- (void)downloadImageWithIndex:(NSUInteger)index {
    ...
    NSURL *URL = [url ttugc_feedImageURL];
    if (URL == nil) {
        URL = [NSURL URLWithString:url];
    }
    WeakSelf;
        //bd
        [[BDWebImageManager sharedManager] requestImage:URL alternativeURLs:nil options:BDImageRequestIgnoreMemoryCache cacheName:nil progress:^(BDWebImageRequest *request, NSInteger receivedSize, NSInteger expectedSize) {
            ...
            if (model == wself.model) {
                if (error || nil == image) {
                    ...
                    [wself downloadImageSuccessWithData:data image:image];
                } else if ([KitchenMgr getBOOL:kTTKUGCPicUsingImageWithoutDataEnabled]) {
                    wself.loadingProgressView.hidden = YES;
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [wself showModel];  //会查看缓存
                    });
                }
            }
        }];
...
- (void)downloadImageSuccessWithData:(NSData *)data {
    [self downloadImageSuccessWithData:data image:nil];
}

- (void)downloadImageSuccessWithData:(NSData *)data image:(UIImage *)image {
    self.loadingProgressView.hidden = YES;
    
    self.isGIF = data ? [[self class] isGIFWithData:data] : [image isGIF];
    ...
    if (self.isGIF) {
        ...
        self.image = data ? [BDImage imageWithData:data] : image;
        toFrame = [[self class] getImageViewFrameWithImage:self.image
                                             containerSize:self.contentView.size];
        self.photoImageView.image = self.image;
    }
...
- (void)showModel {
    ...
    if (cacheImageURL) {
        //图片已下载
        NSData *data;
        data = [[BDImageCache sharedImageCache] imageDataForKey:cacheImageURL];
        ...
        if (self.isGIF) {
            ...
            self.image = [[BDImageCache sharedImageCache] imageForKey:cacheImageURL];
...
#pragma - mark QRCode
- (void)scanQRCode {
    self.longPressGestureRecognizer.enabled = [TTKitchen getBOOL:kTTKUGCBrowserQRCode];
    if (![TTKitchen getBOOL:kTTKUGCBrowserQRCode]) {
        return;
    }
    ...
    if (self.isGIF || self.image == nil) {
        ...
        if (self.image.size.width > 10.f) {
            NSMutableDictionary *infoParam = [NSMutableDictionary new];
            [infoParam setValue:self.model.imageInfosModel.url forKey:@"url"];
            [infoParam setValue:self.model.imageInfosModel.uri forKey:@"uri"];
...
- (void)longPress:(UILongPressGestureRecognizer *)recognizer
{
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(longPressBrowserCell:)]) {
            [self.delegate longPressBrowserCell:self];
        }
    }
}
...
+ (NSString *)getImageURLOfDiskExistsWithImageInfosModel:(FRImageInfoModel *)imageInfosModel {
    NSURL *URL = [imageInfosModel.url ttugc_feedImageURL];
    NSString *key = nil;
    BOOL isExist = NO;
    key = [[BDWebImageManager sharedManager] requestKeyWithURL:URL];
    isExist = [[BDImageCache sharedImageCache] containsImageForKey:key type:BDImageCacheTypeDisk];
    
    return isExist ? key : nil;
}
...
+ (CGRect)getImageViewFrameWithImage:(UIImage *)image containerSize:(CGSize)containerSize {
    if(nil == image){
        ...
        CGFloat imgViewWidth = maxWidth;
        CGFloat imgViewHeight = maxWidth * imageHeight / imageWidth;
