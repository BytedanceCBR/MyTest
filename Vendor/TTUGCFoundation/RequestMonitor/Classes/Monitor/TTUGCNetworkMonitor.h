...
@class TTUGCRequestMonitorModel;
...
typedef NS_ENUM(NSUInteger, kTTNetworkMonitorForumHomePageStatus) {
    kTTNetworkMonitorForumHomePageDataError = 2012, //话题、专题容器Head接口内部数据解析错误
};


@interface TTUGCNetworkMonitor : NSObject

+ (instancetype)sharedInstance;

- (NSString *)getMonitorServiceForClassName:(NSString *)className;

- (NSString *)getMonitorServiceForURL:(NSString *)url;
