...
#import <TTUGCFoundation/FRPhotoBrowserViewController.h>
#import <TTUGCFoundation/FRPhotoBrowserModel.h>
#import "FRImageInfoModel.h"

#define TTUGCSimpleRichLabelLineHeightScale 1.25
#define kTTUGCSimpleLabelImageLinkString @"com.bytedance.kTTUGCSimpleLabelImageLinkString"
...
- (void)setRichSpanText:(TTRichSpanText *)richSpanText {
    ...
    if (richSpanText != _richSpanText) {
        ...
        _richSpanText = [[_richSpanText replaceWhitelistLinks] replaceImageLinksWithIgnoreFlag:NO];
        if (_autoDetectLinks) {
        ...
        for (TTRichSpanLink *current in richSpanLinks) {
            ...
            if (linkRange.location + linkRange.length <= attrStr.length) {
                ...
                if (current.type == TTRichSpanLinkTypeImage) {
                    [self addLinkToURL:[NSURL URLWithString:kTTUGCSimpleLabelImageLinkString] withRange:linkRange];
                } else {
...
static bool isJustOpen = NO;

- (void)showImageLinkBrowser {
    if (isJustOpen){
        return;
    }
    isJustOpen = YES;
    ...
    NSMutableArray <FRPhotoBrowserModel *> * photoBrowserModels = [NSMutableArray array];

    NSArray<FRImageInfoModel *> *imageInfoModels = [TTRichTextImageInfoModel convertImageInfoModels:[_richSpanText.richSpans imageInfoModelArray]];
    [imageInfoModels enumerateObjectsUsingBlock:^(FRImageInfoModel * _Nonnull imageInfoModel, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([imageInfoModel isKindOfClass:[FRImageInfoModel class]]) {

            FRPhotoBrowserModel * photoBrowserModel = [[FRPhotoBrowserModel alloc] initWithImageInfosModel:imageInfoModel
                                                                                          placeholderImage:nil
                                                                                             originalFrame:[NSValue valueWithCGRect:CGRectZero]];
            [photoBrowserModels addObject:photoBrowserModel];
        }
    }];

    FRPhotoBrowserViewController * photoBrowserVC = [[FRPhotoBrowserViewController alloc] initWithModels:photoBrowserModels
                                                                                              startIndex:0
                                                                                              targetView:nil];
    [photoBrowserVC showPhotoBrowserInViewController:nil];
}
                ...
                switch (result.resultType) {
                    case NSTextCheckingTypeLink:
                        if (self.clickDelegate && [self.clickDelegate respondsToSelector:@selector(ugcDefaultRichLabel:didClickURL:)]) {
                            ...
                            if ([result.URL.absoluteString isEqualToString:kTTUGCSimpleLabelImageLinkString]) {
                                [self showImageLinkBrowser];
                            } else {
