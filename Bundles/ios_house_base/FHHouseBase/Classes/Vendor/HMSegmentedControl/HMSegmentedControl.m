...
- (void)commonInit {
    ...
    self.selectionIndicatorWidth = CGFLOAT_MIN;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            rect = CGRectMake(ceilf(rect.origin.x), ceilf(rect.origin.y + self.segmentEdgeInset.top), ceilf(rect.size.width), ceilf(rect.size.height - self.segmentEdgeInset.top));
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    if (self.borderType & HMSegmentedControlBorderTypeBottom) {
        ...
        borderLayer.backgroundColor = self.borderColor.CGColor;
        borderLayer.cornerRadius = self.selectionIndicatorCornerRadius;
        [backgroundLayer addSublayer: borderLayer];
    }
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            if (self.selectionIndicatorWidth != CGFLOAT_MIN) {
                return CGRectMake(x + self.selectionIndicatorEdgeInsets.left + (sectionWidth - self.selectionIndicatorWidth) / 2, indicatorYOffset, self.selectionIndicatorWidth - (self.selectionIndicatorEdgeInsets.left + self.selectionIndicatorEdgeInsets.right), self.selectionIndicatorHeight);
            } else {
...
- (void)scrollToSelectedSegmentIndex:(BOOL)animated {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        if (self.segmentWidthsArray.count > 0 && self.selectedSegmentIndex < self.segmentWidthsArray.count) {
        ...
        selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (segmentWidth / 2);
    }
