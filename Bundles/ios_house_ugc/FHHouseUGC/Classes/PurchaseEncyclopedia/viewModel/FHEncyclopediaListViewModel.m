...
- (instancetype)initWithWithController:(FHEncyclopediaListViewController *)viewController tableView:(UITableView *)table headerView:(FHEncyclopediaHeader *)header userInfo:(NSDictionary *)userInfo {
    self = [super init];
    if (self) {
...
- (void)requestData:(BOOL)isHead first:(BOOL)isFirst {
    ...
    self.requestTask = [FHHouseUGCAPI requestEncyclopediaListWithCategory:self.categoryId channelid:self.channel_id lastGroupId:groupId behotTime:behotTime loadMore:isHead listCount:10 extraDic:extraDic completion:^(id<FHBaseModelProtocol>  _Nonnull model, NSError * _Nonnull error) {
        ...
        if(encyclopediaModel){
            ...
            NSArray *result = [wself convertModel:encyclopediaModel.items isHead:isHead];
            //            NSMutableArray *result = encyclopediaModel.items ;
            //            [encyclopediaModel.items enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            //                NSMutableDictionary *item = [[NSMutableDictionary alloc] initWithDictionary:obj];
            //                 NSMutableArray *filterWordsArr = @[].mutableCopy;
            //                if ([item.allKeys containsObject:@"filter_words"]) {
            //                    NSArray *arr = item[@"filter_words"];
            //                    [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            //                        EncyclopediaItemFilterWordModel *filtermodel = [[EncyclopediaItemFilterWordModel alloc]initWithDictionary:(NSDictionary *)obj error:nil];
            //                        [filterWordsArr addObject:filtermodel];
            //                    }];
            //                };
            //                [item setObject:filterWordsArr forKey:@"filter_words"];
            ...
            if(isHead){
                ...
                //                [self.dataList enumerateObjectsUsingBlock:^(EncyclopediaItemModel *  _Nonnull cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    for (NSDictionary *item in encyclopediaList) {
        if(item){
            if(isHead){
                ...
                //去重逻辑
                [self removeDuplicaionModel:item];
            }else{
