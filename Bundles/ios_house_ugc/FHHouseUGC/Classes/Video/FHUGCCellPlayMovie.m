...
@interface FHUGCCellPlayMovie ()<TTVDemandPlayerDelegate,TTVPlayVideoDelegate ,TTVCommodityViewDelegate ,TTVCommodityViewMessage,TTVCommodityButtonViewDelegate>
...
@implementation FHUGCCellPlayMovie
...
- (void)play
{
    ...
    FHUGCVideoItem *item = (FHUGCVideoItem *)self.cellEntity;
    
    FHFeedContentModel *originData = (FHFeedContentModel *)item.ugcFeedContent;
    //TTVPlayerModel
    TTVVideoPlayerModel *model = [[TTVVideoPlayerModel alloc] init];
    model.categoryID = self.cellEntity.categoryId;
    model.groupID = originData.groupId;
    model.itemID = originData.itemId;
    model.aggrType = [originData.aggrType integerValue];
//    model.adID = article.adId;
//    model.logExtra = article.logExtra;
    model.videoID = originData.videoId;
    model.sp = sp;
//    model.enterFrom = [self enterFrom];
    model.categoryName = self.cellEntity.categoryId;
    model.authorId = originData.userInfo.userId;
    model.defaultResolutionType = TTVPlayerResolutionTypeHD;
    
    NSDictionary *dic = originData.logPb;
    ...
    BOOL isAutoPlaying = NO;
    model.isAutoPlaying = isAutoPlaying;
    model.showMutedView = isAutoPlaying;
    if (isAutoPlaying) {
        //广告自动播放时每次从头播放
        [[TTVPlayerCacheProgressController sharedInstance] removeCacheForVideoID:originData.videoId];
    ...
    playVideo.player.enableRotate = !originData.showPortrait;
    
    NSDictionary *videoLargeImageDict = [[originData.largeImageList firstObject] toDictionary];
    ...
    if (isAutoPlaying && self.cellEntity.article.adId.longLongValue > 0) {
        ...
        self.movieView.player.muted = YES;
    }
    [self addUrlTrackerOnPlayer:playVideo];
    [self settingMovieView:self.movieView];
    [self.movieView.player play];
    [playVideo.player setVideoTitle:originData.title];
    [playVideo.player setVideoWatchCount:[originData.videoDetailInfo.videoWatchCount integerValue] playText:@"次播放"];
