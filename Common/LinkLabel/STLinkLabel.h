 ...
 */
static inline BOOL STLocationInRange(NSRange range, NSUInteger location) {
    return (location >= range.location) && (location <= (range.location + range.length));
}
/**
 * @abstract range1 是否包含range2
 */
static inline BOOL STRangeContainsRange(NSRange range1, NSRange range2) {
...
@property (nonatomic, assign) CGFloat lineSpacing;//default 0
