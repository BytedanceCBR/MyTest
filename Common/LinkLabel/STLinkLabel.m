...
static inline CGRect STRunGetRect(CTLineRef line, CTRunRef run, CGPoint lineOrigin) {
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if ([self hasLinkObjectAtPoint:point]) {
        return self;
    }
    return nil;
}
        ...
        [regexRanges enumerateObjectsWithOptions:NSEnumerationReverse
                                      usingBlock:^(NSTextCheckingResult *result, NSUInteger idx, BOOL *stop) {
                                          ...
                                          if (rangeCount >= 2) {
                                              ...
                                              NSString *backgroundColorString = attrs[@"backgroundColor"];
...
#pragma clang diagnostic ignored "-Wunguarded-availability"
    [paragraphStyle setParagraphStyle:[STLinkLabel paragraphStyleWithFont:self.font lineSpacing:self.lineSpacing]];
...
- (void)sizeToFit {
    CGSize size = [self.displayText sizeWithFont:self.font
                               constrainedToSize:CGSizeMake(self.width, 9999)
                                  ...
                                  paragraphStyle:[[self class] paragraphStyleWithFont:self.font lineSpacing:self.lineSpacing]];
...
+ (NSParagraphStyle *)paragraphStyleWithFont:(UIFont *)font lineSpacing:(CGFloat)spacing {
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
...
+ (NSString *)displayTextWithTextCheckingType:(NSTextCheckingType)textCheckingTypes text:(NSString *)text {
    ...
    NSMutableString *displayString = [NSMutableString stringWithString:textStr];
...
+ (CGSize)sizeWithText:(NSString *)linkText font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineSpacing:(CGFloat)spacing paragraphStyle:(NSParagraphStyle *)paragraphStyle {
    return [self sizeWithText:linkText textCheckingTypes:NSTextCheckingAllSystemTypes | STTextCheckingTypeCustomLink font:font constrainedToSize:constrainedSize lineSpacing:spacing paragraphStyle:paragraphStyle];
}


+ (CGSize)sizeWithText:(NSString *)linkText textCheckingTypes:(NSTextCheckingTypes)checkingTypes font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineSpacing:(CGFloat)spacing paragraphStyle:(NSParagraphStyle *)paragraphStyle  {
    NSString *displayString = [self displayTextWithTextCheckingType:checkingTypes text:linkText];
    return [displayString sizeWithFont:font constrainedToSize:constrainedSize lineSpacing:spacing paragraphStyle:nil];
}
...
- (CGSize)sizeWithFont:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineSpacing:(CGFloat)spacing paragraphStyle:(NSParagraphStyle *)paragraphStyle {
    if (self.length == 0) {
        return CGSizeZero;
    }
    ...
    // 排版
    if (!paragraphStyle) {
        paragraphStyle = [STLinkLabel paragraphStyleWithFont:font lineSpacing:spacing];
    }
