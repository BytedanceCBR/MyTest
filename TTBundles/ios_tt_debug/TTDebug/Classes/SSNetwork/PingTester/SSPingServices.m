...
//    round-trip min/avg/max/stddev = 4.445/9.496/12.210/2.832 ms
    ...
    [pingItems enumerateObjectsUsingBlock:^(SSPingItem *obj, NSUInteger idx, BOOL *stop) {
        if (obj.status == SSPingStatusDidReceivePacket) {
            receivedCount ++;
        }
    }];
...
@interface SSPingServices () <SimplePingDelegate> {
    BOOL _hasStarted;
    BOOL _isTimeout;
...
@property(nonatomic, strong)void(^callbackHandler)(SSPingItem *item, NSArray *pingItems);

@end

@implementation SSPingServices
...
+ (void)getIPAddressWithDomain:(NSString *)domain completionHandler:(void(^)(NSString *IPAddress, NSInteger errorNumber))completionHandler {
    ...
    SimplePing *ping = [SimplePing simplePingWithHostName:domain];
    ping.IPReverseHandler = ^(NSString *IPAddress, NSInteger errorNumber){
        completionHandler(IPAddress, errorNumber);
    };
    [ping start];
}

+ (SSPingServices *)startPingAddress:(NSString *)address
                      callbackHandler:(void(^)(SSPingItem *item, NSArray *pingItems))handler {
    SSPingServices *services = [[SSPingServices alloc] initWithAddress:address];
...
- (void)_timeoutActionFired {
    SSPingItem *pingItem = [[SSPingItem alloc] init];
    pingItem.ICMPSequence = _icmpSequence;
    pingItem.originalAddress = self.address;
    pingItem.status = SSPingStatusDidTimeout;
    [self _handlePingItem:pingItem];
}

- (void)_handlePingItem:(SSPingItem *)pingItem {
    if (pingItem.status == SSPingStatusDidReceivePacket || pingItem.status == SSPingStatusDidTimeout) {
...
- (void)cancel {
    [self.simplePing stop];
    SSPingItem *pingItem = [[SSPingItem alloc] init];
    pingItem.status = SSPingStatusFinished;
    if (self.callbackHandler) {
        self.callbackHandler(pingItem, [_pingItems copy]);
    }
...
- (void)simplePing:(SimplePing *)pinger didSendPacket:(NSData *)packet ICMPHeader:(ICMPHeader *)_ICMPHeader {
    ...
    pingItem.status = SSPingStatusDidStart;
...
- (void)simplePing:(SimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet timeElasped:(NSTimeInterval)timeElasped {
    ...
    pingItem.ICMPSequence = _icmpSequence;
    pingItem.originalAddress = self.address;
    pingItem.status = SSPingStatusDidReceivePacket;
    [self _handlePingItem:pingItem];
}
@end
