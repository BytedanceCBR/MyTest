...
typedef NS_ENUM(NSUInteger, TTVPasterADStyle)
{
    TTVPasterADStyleImage = 1, //图片
    TTVPasterADStyleVideo = 2, //视频
};

typedef NS_ENUM(NSUInteger, TTVPasterADPageType)
{
    TTVPasterADPageTypeAPP = 1,    // 应用下载
    TTVPasterADPageTypeWeb = 2,    // web
};

@class TTVPasterADInfoModel;
@interface TTVPasterADModel : NSObject

@property (nonatomic, assign)   TTVPasterADStyle style;//资源类型

@property (nonatomic, assign)   TTVPasterADPageType type;

@property (nonatomic, strong)   TTVPasterADInfoModel *videoPasterADInfoModel;
...
@interface TTVPasterADInfoModel : JSONModel
...
@property (nonatomic, strong)   TTVPasterADVideoInfoModel <Optional> *videoInfo;//视频相关
