...
#pragma mark - _TTHTSHeaderScrollViewDelegateForwarder

@interface _TTHTSHeaderScrollViewDelegateForwarder : NSObject <TTHTSHeaderScrollViewDelegate>

@property (nonatomic,weak) id<TTHTSHeaderScrollViewDelegate> delegate;

@end

#pragma mark - TTHTSHeaderScrollView

@interface TTHTSHeaderScrollView () <UIGestureRecognizerDelegate>

@property (nonatomic, strong) _TTHTSHeaderScrollViewDelegateForwarder *delegateForwarder;
@property (nonatomic, strong) NSMutableArray<__kindof UIScrollView *> * observedScrollViews;

@end

@implementation TTHTSHeaderScrollView {
    BOOL _isObserving;
    BOOL _lock;
}

static void * const kTTHTSHeaderScrollViewKVOContext = (void*)&kTTHTSHeaderScrollViewKVOContext;
...
- (void)dealloc {
    @try {
        [self removeObserver:self forKeyPath:NSStringFromSelector(@selector(contentOffset)) context:kTTHTSHeaderScrollViewKVOContext];
        [_headerView.layer removeObserver:self forKeyPath:@"bounds" context:kTTHTSHeaderScrollViewKVOContext];
...
- (void)initialize {
    self.delegateForwarder = [_TTHTSHeaderScrollViewDelegateForwarder new];
    ...
    [self addObserver:self
           forKeyPath:NSStringFromSelector(@selector(contentOffset))
              ...
              context:kTTHTSHeaderScrollViewKVOContext];
    _isObserving = YES;
}
...
- (void)setDelegate:(id<TTHTSHeaderScrollViewDelegate>)delegate {
    self.delegateForwarder.delegate = delegate;
    super.delegate = nil;
    super.delegate = self.delegateForwarder;
}

- (id<TTHTSHeaderScrollViewDelegate>)delegate {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if (context == kTTHTSHeaderScrollViewKVOContext && [keyPath isEqualToString:NSStringFromSelector(@selector(contentOffset))]) {
        ...
        if (object == self) {
            ...
            if (!_lock && ((self.contentOffset.y > -self.contentInset.top) || self.bounces)) {
                ...
                if (scrollView.isDecelerating && !self.isDecelerating) {
                    CGFloat y = self.contentOffset.y - diff;
                    if (NO == self.bounces && y < -self.contentInset.top) {
                        y = -self.contentInset.top;
                    }
                    [self scrollView:self setContentOffset:CGPointMake(self.contentOffset.x, y)];
                }
            }
        }
    }
    else if (context == kTTHTSHeaderScrollViewKVOContext && [keyPath isEqualToString:@"bounds"]){
...
@implementation _TTHTSHeaderScrollViewDelegateForwarder
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    [(TTHTSHeaderScrollView *)scrollView scrollViewDidScroll:scrollView];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    [(TTHTSHeaderScrollView *)scrollView scrollViewDidEndDecelerating:scrollView];
    if ([self.delegate respondsToSelector:_cmd]) {
        [self.delegate scrollViewDidEndDecelerating:scrollView];
    }
}
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    [(TTHTSHeaderScrollView *)scrollView scrollViewDidEndDragging:scrollView willDecelerate:decelerate];
    if ([self.delegate respondsToSelector:_cmd]) {
