...
@interface WDWendaListQuestionHeader()
...
@property (nonatomic, strong) WDListQuestionHeaderDescView *descView;
@property (nonatomic, strong) WDListQuestionHeaderAnswerView *answerCountView;

@end

@implementation WDWendaListQuestionHeader
...
- (CGFloat)layoutOldVersionViews:(CGFloat)originY
{
    ...
    originY = SSMaxY(self.answerCountView);
    return originY;
}
...
- (CGFloat)descPadding
{
    return isEmptyString(self.viewModel.questionEntity.content.text) ? WDPadding(10) : WDPadding(6);
}
...
- (WDListQuestionHeaderDescView *)descView
{
    if (!_descView) {
        CGFloat y = SSMaxY(self.titleView) + [self descPadding];
        _descView = [[WDListQuestionHeaderDescView alloc] initWithFrame:CGRectMake(0, y, SSWidth(self), 0) viewModel:self.viewModel];
