...
static WDDislikeView *__visibleDislikeView;

@implementation WDDislikeViewModel

@end

#pragma mark - WDDislikeView

@interface WDDislikeView () <TTFeedDislikeKeywordsViewDelegate>
...
@property(nonatomic, strong)WDDislikeClickBlock didDislikeBlock;

@end

@implementation WDDislikeView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.titleLabel.text = NSLocalizedString(@"不想回答，精准屏蔽", nil);
...
- (void)refreshTitleLabel
{
    if (self.selectedWords.count > 0) {
        ...
        [self.titleLabel setText:@"不想回答，精准屏蔽"];
    }
...
- (CGFloat)arrowOffsetY {
    static float gapY = 0;
    if (gapY == 0) {
        if ([TTDeviceHelper is667Screen]) { // iPhone6
            gapY = 8.f;
