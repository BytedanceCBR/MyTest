...
@interface TTVPlayerControlBottomView () <TTVPlayerControlSliderViewDelegate>
...
@implementation TTVPlayerControlBottomView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _fullScreenButton.exclusiveTouch = YES;
        self.isFull = NO;
...
- (void)ttv_kvo
{
    ...
    [self.KVOController observe:self.playerStateStore.state keyPath:@keypath(self.playerStateStore.state,currentResolution) options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        ...
        if ([TTVResolutionStore sharedInstance].resolutionAlertClick) {
            [self setResolutionString:[NSString stringWithFormat:@"自动"]];
        }else if ([TTVResolutionStore sharedInstance].userSelected && ![TTVResolutionStore sharedInstance].forceSelected) {
