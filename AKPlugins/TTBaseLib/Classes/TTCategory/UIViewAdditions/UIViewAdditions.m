...
- (UIEdgeInsets)tt_safeAreaInsets{
    ...
    if ((viewFrameInWindow.origin.y < 40 && ![UIApplication sharedApplication].statusBarHidden) || statusBarView.height <= 0 ){
        if (safeInset.top <= 0){
...
- (NSArray *)viewWithClass:(Class) aClass {
    ...
    [self _subviewFromView:self ofClass:aClass array:subviews];
    return [subviews copy];
}


/// 私有方法，递归查找subvie类型为class的所有subview
- (void)_subviewFromView : (UIView *)view ofClass : (Class) aClass array : (NSMutableArray *)array {
    NSArray *subviews = view.subviews;
    [subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:aClass]) {
            [array addObject:obj];
        }
        [self _subviewFromView:obj ofClass:aClass array:array];
    }];
}
