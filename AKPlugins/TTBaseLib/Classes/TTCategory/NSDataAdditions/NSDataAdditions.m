...
NSString * const kRSADecryptionErrorDomain = @"kRSADecryptionErrorDomain";

@implementation NSData(TTAdditions)
...
+ (SecKeyRef)addPublicKey:(NSString *)key error:(NSError **)error {
    ...
    if(!data){
        if (error) {
            *error = [NSError errorWithDomain:kRSADecryptionErrorDomain code:1000 userInfo:@{@"description" : @"stripPublicKeyHeader"}];
        }
        return nil;
    }
    ...
    if ((status != noErr) && (status != errSecDuplicateItem)) {
        if (error) {
            *error = [NSError errorWithDomain:kRSADecryptionErrorDomain code:status userInfo:@{@"description" : @"SecItemAdd"}];
        }
        return nil;
    }
    ...
    if(status != noErr){
        if (error) {
            *error = [NSError errorWithDomain:kRSADecryptionErrorDomain code:status userInfo:@{@"description" : @"SecItemCopyMatching"}];
        }
        return nil;
    }
    return keyRef;
...
+ (NSData *)decryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef error:(NSError **)error {
    ...
    for(int idx=0; idx<srclen; idx+=src_block_size){
        ...
        if (status != 0) {
            ...
            if (error) {
                *error = [NSError errorWithDomain:kRSADecryptionErrorDomain code:status userInfo:@{@"description" : @"SecKeyDecrypt"}];
            }
            ret = nil;
            break;
        }else{
...
+ (NSData *)encryptData:(NSData *)data withKeyRef:(SecKeyRef)keyRef error:(NSError **)error{
    ...
    for(int idx=0; idx<srclen; idx+=src_block_size){
        ...
        if (status != 0) {
            if (error) {
                *error = [NSError errorWithDomain:kRSADecryptionErrorDomain code:status userInfo:@{@"description" : @"SecKeyEncrypt"}];
            }
            NSLog(@"SecKeyEncrypt fail. Error Code: %d", (int)status);
            ret = nil;
            break;
...
- (NSData *)tt_dataWithFingerprintType:(TTFingerprintType)type {
    ...
    UInt8 bytes[self.length];
    [self getBytes:&bytes length:self.length];
    UInt8 fingers[4] = {0x9d, 0x8e, 0x7f, 0x5a};
    for (int i = 0; i < self.length; i ++) {
        NSInteger finger = 0x9D;
        if (type == TTFingerprintTypeEnumXOR) {
            finger = fingers[i % 4];
        }
        bytes[i] ^= finger;
    }
    return [NSData dataWithBytes:bytes length:self.length];
}
...
@implementation NSData (JSONExtension)

- (id)JSONValue {
    return [self.class _objectWithJSONData:self error:nil];
}

+ (id)_objectWithJSONData:(NSData *)inData error:(NSError **)outError {
    if (!inData) {
        return nil;
    }
    NSError *error = nil;
    id object = [NSJSONSerialization JSONObjectWithData:inData options:NSJSONReadingAllowFragments error:&error];
    if (error) {
        return nil;
    }
    return object;
