...
@implementation TTSearchBarView
...
- (id)initWithFrame:(CGRect)frame {
    if (frame.size.width < TTSearchBarViewDefaultSize.width) {
        frame.size.width = TTSearchBarViewDefaultSize.width;
    }
    if (frame.size.height < TTSearchBarViewDefaultSize.height) {
        frame.size.height = TTSearchBarViewDefaultSize.height;
    }
    ...
    if (self) {
        ...
        self.inputBackgroundView = [[SSThemedButton alloc] initWithFrame:CGRectMake(kSearchBarLeftPad, 6, contentFrame.size.width - kSearchBarLeftPad*2, contentFrame.size.height - 12)];
        ...
        self.inputBackgroundView.isAccessibilityElement = NO;
        ...
        self.closeButton.frame = CGRectMake((self.inputBackgroundView.width) - 22, (self.inputBackgroundView.height - 14) / 2.f, 14, 14);
        ...
        // 调整输入框大小->输入框被遮挡
        CGFloat searchFieldHeight = self.inputBackgroundView.height - 8;
        CGFloat searchFieldY = (self.inputBackgroundView.height - searchFieldHeight) / 2.f;
        self.searchField = [[SSThemedTextField alloc] initWithFrame:CGRectMake(30, searchFieldY, self.closeButton.left - 15 - self.searchImageView.right, searchFieldHeight)];
        ...
        self.searchField.isAccessibilityElement = YES;
        self.searchField.accessibilityTraits = UIAccessibilityTraitSearchField;
...
- (void)clearSearchText:(id)sender {
    ...
    /////// 友盟统计
    [BDTrackerProtocol event:@"search_tab" label:@"clear_input"];
    if ([self.delegate respondsToSelector:@selector(searchBar:textDidChange:)]) {
        [self.delegate searchBar:self textDidChange:nil];
    }
    if ([self.delegate respondsToSelector:@selector(searchBarTextDidClear:)]) {
        [self.delegate searchBarTextDidClear:self];
    }
}
...
CGSize const TTSearchBarViewDefaultSize = {320, 44};
