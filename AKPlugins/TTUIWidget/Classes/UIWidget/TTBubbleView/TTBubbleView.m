...
@property (nonatomic, assign) CGFloat imageViewHeightOffset;
                 ...
                 paddingH:(CGFloat)paddingH
{
    if(self = [super initWithFrame:CGRectZero]){
        ...
        self.containerHeight = containerViewHeight;
        self.imageViewHeightOffset = imageViewHeightOffset;
        self.paddingH = paddingH;
        [self commonInit];
    }
    return self;
...
- (void)buildViews{
    ...
    if (self.bgImage) {
        [self buildContainerViewWithBgImage:maxContainerWidth];
    ...
    if (self.left < screenMargin || self.right > screenWidth - screenMargin) {//left,right限制在屏幕15pi内
        ...
        self.centerX += offset;
        self.upArrowView.centerX = self.containerView.centerX - offset;
        self.downArrowView.centerX = self.containerView.centerX - offset;
        self.line.centerX -= offset;
        self.dot.centerX -= offset;
        
        // 在修改为整体绘制之前的workaround
        if(self.arrowDirection == TTBubbleViewArrowUp){
            [self setBubbelAnchorPointTo:CGPointMake((self.containerView.centerX - offset)/self.width, 0.f)];
        }
        else{
            [self setBubbelAnchorPointTo:CGPointMake((self.containerView.centerX - offset)/self.width, 1.f)];
        }
    }
}

- (void)setBubbelAnchorPointTo:(CGPoint)newAnchorPoint {
    CGPoint newPoint = CGPointMake(self.bounds.size.width * newAnchorPoint.x, self.bounds.size.height * newAnchorPoint.y);
...
animationCompleteHandle:(void(^)(void))animationCompletionHandle
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:0.45 delay:delay usingSpringWithDamping:0.78f initialSpringVelocity:0.f options:UIViewAnimationOptionCurveEaseIn animations:^{
...
{
    [self showTipWithAnimation:animation
                     animStyle:TTBubbleViewAnimStyleDefault
                 automaticHide:automaticHide
              autoHideInterval:autoHideInterval
       animationCompleteHandle:animationCompletionHandle
                autoHideHandle:autoHideHandle
                     tapHandle:tapHandle
                   closeHandle:closeHandle
                  shouldShowMe:shouldShowMeHandler];
}
...
- (void)showTipWithAnimation:(BOOL)animation
     ...
     animationCompleteHandle:(void (^)(void))animationCompletionHandle
              autoHideHandle:(void (^)(void))autoHideHandle
                   tapHandle:(void (^)(void))tapHandle
                 closeHandle:(void (^)(void))closeHandle
                shouldShowMe:(TTDoAskForShowDialogBlock)shouldShowMeHandler
{
    if (_isAnimating || _isShowing) {
        return;
    }
    ...
    [TTDialogDirector enqueueShowDialog:self withPriority:TTDialogPriorityTip shouldShowMe:^BOOL(BOOL * _Nullable keepAlive) {
        ...
        if (animation) {
            ...
            [self executeAnim:animStyle animationCompleteHandle:animationCompletionHandle];
        } else {
            self.isAnimating = NO;
            self.userInteractionEnabled = YES;
            self.dot.transform = CGAffineTransformIdentity;
...
- (void)executeAnim:(TTBubbleViewAnimStyle)animStyle animationCompleteHandle:(void (^)(void))animationCompletionHandle {
    if (animStyle == TTBubbleViewAnimStyleWithBgImage) {
        self.containerView.transform = CGAffineTransformMake(0.01, 0, 0, 0.01, 0, self.bounds.size.height);
        self.upArrowView.transform = CGAffineTransformMake(0.01, 0, 0, 0.01, 0, -self.bounds.size.height);
        self.downArrowView.transform = CGAffineTransformMake(0.01, 0, 0, 0.01, 0, self.bounds.size.height);
...
- (void)buildViewForArrowUp{
    ...
    if (self.lineHeight <= 0) {
        ...
        if (self.arrowImage) {
            self.upArrowView.layer.contents = (id) self.arrowImage.CGImage;
        } else {
...
- (void)buildViewForArrowDown{
    ...
    if (self.lineHeight <= 0) {
        ...
        if (self.arrowImage) {
            self.downArrowView.layer.contents = (id) self.arrowImage.CGImage;
        } else {
...
- (void)buildContainerViewWithBgImage:(CGFloat)maxContainerWidth {
    ...
    self.bgImageView.width = self.containerView.width;
    self.bgImageView.height = self.containerView.height + self.imageViewHeightOffset;
    
    [self.bgImageView addSubview:self.tipTextLabel];
    [self.containerView addSubview:self.bgImageView];
    
    self.tipTextLabel.left = leftPadding;
    self.tipTextLabel.centerY = self.bgImageView.centerY;
}
