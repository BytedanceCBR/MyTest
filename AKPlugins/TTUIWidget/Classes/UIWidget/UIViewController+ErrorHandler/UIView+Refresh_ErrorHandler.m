...
- (UIView*)ttLoadingTargetView {
    return (UIView*)objc_getAssociatedObject(self, @selector(ttLoadingTargetView));
}

- (void)setTtLoadingTargetView:(UIView *)ttLoadingTargetView {
    objc_setAssociatedObject(self, @selector(ttLoadingTargetView),ttLoadingTargetView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (BOOL)ttHiddenLoadingView {
    NSNumber *number = objc_getAssociatedObject(self, @selector(ttHiddenLoadingView));
    return [number boolValue];
}

- (void)setTtHiddenLoadingView:(BOOL)ttHiddenLoadingView {
    objc_setAssociatedObject(self, @selector(ttHiddenLoadingView),@(ttHiddenLoadingView), OBJC_ASSOCIATION_RETAIN_NONATOMIC) ;
}
...
- (void)tt_startUpdate {
    ...
    if (![target performSelector:@selector(tt_hasValidateData)]) {
        ...
        if (self.ttLoadingTargetView) {
            [self.ttLoadingTargetView addSubview:self.ttLoadingView];
        } else {
