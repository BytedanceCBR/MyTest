...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (self.bounces && _lock && velocity.y < 0) {
            //目前的bounces是完全不可用的，一旦contentView是一个tableview，上滑会出现下方的灰块
            return NO;
        }
    }
    return YES;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if (context == kTTHeaderScrollViewKVOContext && [keyPath isEqualToString:NSStringFromSelector(@selector(contentOffset))]) {
        ...
        if (object == self && _resettingBottom && old.y != 0) {
            _resettingBottom = NO;
            return;
        }
        ...
        if (object == self) {
            ...
            } else if (diff > 0 && _lock && self.bounces) {
                _resettingBottom = YES;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.contentOffset.y > -self.headerView.minimumHeaderHeight && self.autoResetHeader) {
        [self scrollView:self setContentOffset:CGPointMake(self.contentOffset.x, -self.headerView.minimumHeaderHeight)];
    }
}
