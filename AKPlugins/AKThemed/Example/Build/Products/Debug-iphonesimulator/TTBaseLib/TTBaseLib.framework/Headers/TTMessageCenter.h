 ...
 *   REGISTER_MESSAGE后，关注者会被引用，而一般在关注者析构时才会UNREGISTER_MESSAGE。
     但因为被引用了，没办法掉到析构。
     所以引入TTMessageObject， 不再对关注者加引用。但带来其他问题：
     如果关注者忘记UNREGISTER_MESSAGE， 则会crash。
 */
@interface TTMessageObject : NSObject
{
    CFTypeRef m_Obj;
}
...
@interface TTMessageCenter : TTService <TTService>
{
    // map(TTMessageKey -> TTMessage)
    NSMutableDictionary *m_dicMessage;
}

- (TTMessage *)getMessage:(TTMessageKey)key;
...
{ \
    TTMessage *__oMessage__ = [GET_SERVICE(TTMessageCenter) getMessage:@protocol(message)]; \
    if (__oMessage__) { \
