...
def bd_enable_DevelopMode
  if File::exist?('Podfile_Patch') && !ENV['WORKSPACE']
    load 'Podfile_Patch'
  end
...
def getDeprecatedContainerByApp(apps=["13"],moduleName)
  raise '不应该在Podfile_Patch使用pod_source or pod_binary' unless apps
  versions = Array.new
  apps.each do |app|
    ...
    def ensure_plugins_are_installed!
      ...
      podfile.plugins.keys.each do |plugin|
        unless loaded_plugins.include? plugin
          raise Informative, "Your Podfile requires that the plugin `#{plugin}` be installed. Please install it and try installation again." if plugin != 'cocoapods-bytedance-transform'
        end
      end
    end
  end
