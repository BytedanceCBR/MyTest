...
- (void)layoutForXiGuaStyle {
    ...
    UIView * speed = [self partControlForKey:TTVPlayerPartControlKey_SpeedChangeButton];
    BOOL isSimple = YES;
    ...
    UIView * playBottom = [self partControlForKey:TTVPlayerPartControlKey_PlayBottomToggledButton];
    UIView * currentAndTotal = [self partControlForKey:TTVPlayerPartControlKey_TimeCurrentAndTotalLabel];
    ...
    if (fullScreen && !isSimple) {
        ...
        slider.width = self.view.width - leftEdge * 2;
        ...
        // speed
        CGRect rect = [slider.superview convertRect:slider.frame toView:self.view];
        [speed sizeToFit];
        speed.width = speed.width;
        speed.height = 34;
        speed.right = slider.right;
        speed.centerY = (self.view.height - CGRectGetMaxY(rect)) / 2.0 + CGRectGetMaxY(rect);
    }else{
