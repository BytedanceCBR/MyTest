...
@implementation AlertSystemCustom
...
- (void)customIOS7dialogButtonTouchUpInside: (AlertSystemCustom *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    [self close];
}
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i = 0; i < [buttonTitles count]; i++) {
        ...
        if (i < buttonTitles.count - 1) {
            ...
            sep.frame = CGRectMake(CGRectGetMaxX(closeButton.frame),CGRectGetMinY(closeButton.frame), 1, CGRectGetHeight(closeButton.frame));
            sep.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
...
@implementation TTImagePickerAlert
...
+ (void)showWithTitle:(NSString *)title
{
    ...
    if (title.length > 9) {
        label.frame = CGRectMake((kScreenWidth - 200) / 2, kScreenHeight/2 - 32, 200, 64);
        label.font = [UIFont systemFontOfSize:12];
        label.numberOfLines = 3;
    }else
    {
        label.frame = CGRectMake((kScreenWidth - 150)/2, kScreenHeight/2 - 22, 150, 44);
