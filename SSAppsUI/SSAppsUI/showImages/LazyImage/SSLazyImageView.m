...
@property (nonatomic, retain, readwrite) UIImageView * netImageView;
@property (nonatomic, retain) NSOperationQueue * operationQueue;
@property (nonatomic, retain) ASIHTTPRequest * imageRequest;

@property (nonatomic, retain) SSImageInfosModel * imageModel;
@property (nonatomic, assign) NSInteger imageModelCurrentDownloadIndex;
...
@synthesize imageModelCurrentDownloadIndex = _imageModelCurrentDownloadIndex;
...
- (id)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        _imageModelCurrentDownloadIndex = 0;
...
- (void)setFrame:(CGRect)frame
{
    ...
    BOOL onlyChangeOriginal = NO;
    if (CGSizeEqualToSize(frame.size, _netImageView.frame.size)) {
        onlyChangeOriginal = YES;
    }
    
    _netImageView.frame = self.bounds;
//    _borderView.frame = self.bounds;
    
    if (onlyChangeOriginal) {
        return;
    }
...
{    
    ...
    if ([SSImageInfosModel isImageInfosModel:model equalesToModel:_imageModel]) {
        isChanged = NO;
    }
...
{
    SSImageInfosModel * model = [[SSImageInfosModel alloc] initWithURL:URLstring withHeader:header];
    [self setNetImageInfosModel:model];
    [model release];
}

- (void)setNetImageUrl:(NSString *)URLString
{
    [self setNetImageURL:URLString withHeader:nil];
}
...
- (void)setLayerConerRadius:(CGFloat)radius
{
    [self.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[UIView class]]) {
            ((UIView *)obj).layer.cornerRadius = radius;
            ((UIView *)obj).clipsToBounds = YES;
        }
    }];
}
...
{
    if ((_requestOnLoading && [_imageModel.urlWithHeader count] > 0) || [_imageModel.urlWithHeader count] == 0) {
        return;
    }
    
    NSData * data = [[SSSimpleCache sharedCache] dataForImageInfosModel:_imageModel];
...
- (void)downloadImage
{
    if (!_requestOnLoading) {
        ...
        NSString * urlStr = nil;
        if ([self isImageModelIndexlegal]) {
            urlStr = [[_imageModel.urlWithHeader objectAtIndex:_imageModelCurrentDownloadIndex] objectForKey:SSImageInfosModelURL];
        }
        else {
            SSLog(@"illegal _imageModelCurrentDownloadIndex %i, _imageModel.urlWithHeader count %i", _imageModelCurrentDownloadIndex, [_imageModel.urlWithHeader count]);
        }
        ...
        if (url != nil) {
            ...
            self.imageRequest = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:[urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
        }
...
{
//        UIImage * imageData_ = nil;
//        if (imageData.imageOrientation != UIImageOrientationUp) {
//            imageData_ = [UIImage imageWithCGImage:imageData.CGImage scale:1.f orientation:UIImageOrientationUp];
//            self.netImage = imageData_;
//        }
//        else {
//            self.netImage = imageData;
//        }
//    return;
...
- (void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    if(SSNetworkConnected())
    {
        ...
        if(start > 0)
        {
            ...
            [SSTracker sendCDNWithRequest:request duration:duration];
            start = 0; 
        }
    }

    _imageModelCurrentDownloadIndex ++;
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    {
        NSTimeInterval duration = [[NSDate date] timeIntervalSince1970] - start;
        if ([SSCommon random:2] == 1 && [SSLazyImageView sampledNumber] < kSampleNumberForDownloadTime) {
            int connect = 4;
            if(!SSNetworkWifiConnected())
            {
                connect = 1;
    ...
    if (responseData != nil) {
        NSString * key = _imageModel.URI;
        
        if ([key length] == 0) {
            key = request.url.absoluteString;
        }
        
        [[SSSimpleCache sharedCache] setData:responseData forKey:key];
    }
...
- (BOOL)isImageModelIndexlegal
{
    return _imageModelCurrentDownloadIndex < [_imageModel.urlWithHeader count];
}
